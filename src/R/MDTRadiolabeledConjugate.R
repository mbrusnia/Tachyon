...
for(i in 1:ndata){
    SPECIFIC_ACTIVITY[i] = round(as.numeric(inputDF[i,ELUTE_CPM])/as.numeric(inputDF[i,INPUT_PROTEIN_MG])*as.numeric(inputDF[i,ELUTE_ML])*100.0*CONVERSION_CONST, digit=2)
}
result <- cbind(inputDF, SPECIFIC_ACTIVITY)
names(result) <- c("ChemProduction ID", "Input Protein (mg)", "Elute CPM", "Elute (mL)", "Comment", "Specific Activity uCi/mg" )
