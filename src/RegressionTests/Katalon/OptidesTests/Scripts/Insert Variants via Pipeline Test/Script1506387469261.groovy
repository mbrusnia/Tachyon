...
WebUI.click(findTestObject('Page_Assay Dashboard OptidesCompoun/label_Insert New Variant Seq'))
...
if(WebUI.verifyElementVisible(findTestObject('input_ProtocolName'), FailureHandling.OPTIONAL)){
	WebUI.setText(findTestObject('input_ProtocolName'), 'TEST - Variants insert Via Pipeline')
}
...
while ((pipelineStatus == 'insert_variants RUNNING') || (pipelineStatus == 'insert_variants WAITING')) {
    WebUI.delay(10)
